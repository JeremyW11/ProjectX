  ...
  container.innerHTML = items.map(function(item) {
    return '<div class="news-item expandable" onclick="toggleExpand(this)">' +
      ...
      '<div class="news-body">' +
        ...
        '<div class="news-footer"><span class="news-time">' + item.time + '</span><span class="news-impact">' + item.impact + '</span></div>' +
        '<div class="news-expand"><div class="expand-divider"></div>' +
          '<div class="expand-row" style="align-items:flex-start;gap:12px;"><span class="expand-label" style="flex-shrink:0;margin-top:2px;">深度分析</span><span class="expand-value" style="text-align:left;line-height:1.6;">' + (item.expand_analysis || item.impact) + '</span></div>' +
          '<div class="expand-row"><span class="expand-label">關注標的</span><span class="expand-value">' + (item.expand_tickers || getRelatedTickers(item.tag)) + '</span></div>' +
          '<div class="expand-row" style="align-items:flex-start;gap:12px;"><span class="expand-label" style="flex-shrink:0;margin-top:2px;">持續追蹤</span><span class="expand-value" style="text-align:left;line-height:1.6;">' + (item.expand_watch || '待觀察') + '</span></div>' +
        '</div>' +
      '</div>' +
  ...
  container.innerHTML = persons.map(function(p) {
    ...
    return '<div class="person-item expandable" onclick="toggleExpand(this)">' +
      ...
      '<div class="person-body">' +
        ...
        '<div class="news-expand"><div class="expand-divider"></div>' +
          '<div class="expand-row"><span class="expand-label">機構</span><span class="expand-value">' + (parts[1] || p.title) + '</span></div>' +
          '<div class="expand-row"><span class="expand-label">職位</span><span class="expand-value">' + (parts[0] || '—') + '</span></div>' +
          '<div class="expand-row" style="align-items:flex-start;gap:12px;"><span class="expand-label" style="flex-shrink:0;margin-top:2px;">觀察重點</span><span class="expand-value" style="text-align:left;line-height:1.6;">' + (p.expand_watch || p.note) + '</span></div>' +
        '</div>' +
      '</div>' +
      '<div class="expand-arrow">›</div>' +
    '</div>';
...
function renderSignals(signals) {
  ...
  container.innerHTML = signals.map(function(s) {
    return '<div class="industry-row"><div class="ind-left"><div class="ind-dot" style="background:' + dotColor(s.signal) + '"></div><span class="ind-name">' + s.name + '</span></div><span class="signal-pill ' + s.signal + '">' + s.label + '</span></div>';
  }).join('');
}
  ...
  if (sentiment.fear_greed) {
    ...
    if (fgEl) fgEl.innerHTML =
      '<div class="fg-container">' +
        ...
        '<div style="display:flex;justify-content:space-between;font-size:9px;color:var(--text-muted);margin-bottom:10px;"><span>極度恐懼</span><span>恐懼</span><span>中性</span><span>貪婪</span><span>極貪婪</span></div>' +
  ...
  if (sentiment.polymarket && sentiment.polymarket.length > 0) {
    ...
    if (polyEl) polyEl.innerHTML = sentiment.polymarket.map(function(p) {
      ...
      return '<div class="poly-item">' +
        '<div class="poly-header"><div class="poly-title">' + p.title + '</div><div class="poly-prob">' + p.prob + '</div></div>' +
...
function dotColor(signal) {
  var map = { bull: 'var(--accent-green)', bear: 'var(--accent-red)', watch: 'var(--accent-orange)', neutral: 'var(--text-muted)' };
